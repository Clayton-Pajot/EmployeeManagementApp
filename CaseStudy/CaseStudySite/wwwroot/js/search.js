...
//VALIDATION START
    ...
    $("#getbutton").click(async (e) => {
        try {
            let email = $("#srch").val();         //OLD CODE: let lastname = $("#TextBoxFindLastName").val();
            $("#status").text("please wait...");
            $("#myModal").modal("toggle");
            let response = await fetch(`api/employee/${email}`);         //OLD CODE: let response = await fetch(`api/employee/${lastname}`);
            ...
            if (response.ok) {
                ...
                if (data.lastname !== "not found") {//
                    ...
                    $("#ddlDepartments").val(data.departmentid);
                    $("#modalStatus").text("employee found");
                    $("#imageBox").html(`<img height="120" width="110" src="data:img/png;base64,${data.staffPicture64}" />`);
                    sessionStorage.setItem("Id", data.id);
                    sessionStorage.setItem("DepartmentId", data.departmentId);
                    sessionStorage.setItem("Timer", data.Timer);
                    sessionStorage.setItem("StaffPicture", data.staffPicture64);
                } else {
                    $("#TextBoxTitle").val("");
   ...
   /* $("#getbutton").mouseup(async (e) => {
    ...
    $("input:file").change(() => {
        ...
        reader.onload = (readerEvt) => {
            ...
            $("#imageBox").html(`<img height="120" width="110" src="data:img/png;base64,${encodedString}" />`);
    ...
    const setupForUpdate = (id, data) => {
        ...
        data.map(employee => {
            if (employee.id === parseInt(id)) {
                ...
                $("#imageBox").html(`<img height="120" width="110" src="data:image/png;base64,${employee.staffPicture64}"/>`);
